...
pub const Date = struct {
	...
	fn bufWrite(self: Date, buf: []u8) u8 {
...
test "date: parse" {
...

	{
		...
		try t.expectError(error.InvalidDate, Date.parse("20230022", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("20230A22", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("20231322", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("20239922", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("-20230022", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("-20231322", .iso8601));
		try t.expectError(error.InvalidDate, Date.parse("-20239922", .iso8601));
	}
